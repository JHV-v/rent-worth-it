...
> 看完 AGENTS.md，项目现在在 v1.6.1，访问计数刚稳定下来，下一步路线图列了 v1.6.2 / v1.7.0 / v1.8.0 三个方向。
> 
> 想做小一点的（一晚上）还是大一点的（多天）？给你三个选项：
> 
> A. **v1.6.2 - diagnostics 字段**（推荐，最小闭环）：结果页展示各维度命中了哪些规则。改 `app/lib/score/index.ts` 加 `diagnostics` 输出 + 结果页加一个折叠区。
> 
> B. **v1.7.0 - 方法论页 /methodology**：新建一个静态路由展示算法说明，可以直接复用 README 的"评分算法简介"段落。
> 
